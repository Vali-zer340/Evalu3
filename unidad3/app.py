...
def form1():
    resultado = None  # Inicializa el resultado como None
...
def perfil():
    datos_persona = {
        'nombre' : 'Valiant',
        'edad' : 18,
        'intereses' : ['Videojuegos', 'Programacion']
    }
...
@app.route("/")
def main():
    return 'Bienvenido a la calculadora calculosa, que te calcula hasta las nalgas.'

@app.route('/calcular/<int:numero>')
def calcular(numero):
    cuadrado = calcular_cuadrado(numero)
    cubo = calcular_cubo(numero)
    return render_template('resultado.html', numero= numero, cuadrado=cuadrado,cubo=cubo)
